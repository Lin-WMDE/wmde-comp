...
pub fn setup_socket() -> Result<()> {
    if let Ok(fd_num) = std::env::var("WMDE_SESSION_SOCK")
...
pub fn run_socket(handle: LoopHandle<State>, common: &Common) -> Result<()> {
    if let Ok(fd_num) = std::env::var("WMDE_SESSION_SOCK") {
        if let Ok(fd) = fd_num.parse::<RawFd>() {
                        ...
                        match std::str::from_utf8(&stream.buffer) {
                            Ok(message) => {
                                match serde_json::from_str::<'_, Message>(message) {
                                    ...
                                    _ => warn!("Unknown session socket message, are you using incompatible wmde-session and wmde-comp versions?"),
                                };
            ...
            error!(socket = fd_num, "WMDE_SESSION_SOCK is no valid RawFd.");
        }
    };
